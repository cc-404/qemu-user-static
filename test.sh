...
docker run --platform arm64 --rm -t arm64v8/ubuntu uname -m
# It should install a package.
docker build --platform arm64 --rm -t "test/latest/ubuntu" -<<EOF
FROM arm64v8/ubuntu
RUN apt-get update && \
...
docker run --platform arm64 --rm -t arm64v8/fedora uname -m
# It should install a package.
...
FROM --platform=arm64 arm64v8/ubuntu
